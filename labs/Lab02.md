...
4. Copy the `tpmoveride.json` file to the HummingBoard by replacing the `device IP address` collected earlier from **notes** file in each of the following two commands with the IP Address of your HummingBoard device, then run the two commands in your command prompt window; one command after the other:
```batch
