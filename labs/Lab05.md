...
1. Hit run to build the app and confirm that it works on your local device, the app should open a window with 4 text boxes and a button.
1. Open up the test data CSV that we saved earlier and enter the first four values into the window in order.
1. The result should be within the error range of the fifth value in the data set, the error range is given by the validation steps returned during training 
	* i.e. if you have 455 as an output and 453 with an error of 3.5 the model is working as expected, if the error is much greater there may be an issue.
1. To deply the app on the device, simply change the run target from x86 to ARM and select the device 
